...
class MEG_BR_Trial(Trial):
    ...
    def draw(self, *args, **kwargs):
        ...
        if self.phase == 2:  # stimulus
            ...
            # flicker_level_red = int((self.frame%red_ff)==0)#int(np.floor(self.frame % (red_ff*2) / red_ff))
            # flicker_level_green = int((self.frame%green_ff)==0)#int(np.floor(self.frame % (green_ff*2) / green_ff))
            ...
            if flicker_level_red != self.red_flicker_frame and flicker_level_red == 1:
                self.red_orientation = self.parameters['grating_rotation_speed'] * \
                    pres_time * \
                    self.parameters['motion_direction'] * 360.0  # + 90.0
...
#            self.frame += 1
    ...
    def run(self):
        ...
        if self.parameters['color_eye_combination'] == 1:
            self.session._grating_1.setPos(self.session.right_pos)
